...
4. Siga `database/CONFIGURAR-ADMIN.md` para criar a conta do responsável e aprovar quem precisa de acesso. Qualquer conta aprovada (e ativa) já vê os cadastros automaticamente ao abrir a página Fiscais — não existe mais um login separado dentro dela. "Fiscais" marcada em **Acessos** só controla se essa pessoa vê o card/consegue abrir a página; não é uma segunda trava sobre os dados.
...
- Toda conta aprovada compartilha a mesma base. Use **Atualizar cadastros** para buscar mudanças feitas por outra pessoa. Não há atualização em tempo real automática.
...
- Quem tem acesso pode editar (não só adicionar) os cadastros já existentes, direto no card de cada pessoa. Não há exclusão de registros pelo aplicativo. Quem tem acesso é definido em **Acessos**, só o responsável mexe lá.
